...
public static class Server
{
    ...
    public static bool GetClient(int id, out Client client)
    {
    ...
    public static void Send<T>(int clientId, T packet, DeliveryMethod method, byte channel = 0)
        where T : class, new()
    {
        if (Clients.TryGetValue(clientId, out var client)) client.Send(packet, method, channel);
    ...
    public static void Subscribe<T>(PacketsDispatcher.PacketsSubscription<T> action) where T : class, new()
    {
    ...
    public static void Unsubscribe<T>(PacketsDispatcher.PacketsSubscription<T> action) where T : class, new()
    {
